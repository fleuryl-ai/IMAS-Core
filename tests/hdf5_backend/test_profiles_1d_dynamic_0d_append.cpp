...
int main() {
    try {
        ...
        // 1. Phase 1: Write initial slice (Global Write of 1 element)
        {
            ...
            double val = 100.0;
            // dim=0 for scalar.
            backend.writeData(&profilesCtx, "signal_0d", "time", &val, alconst::double_data, 0, nullptr);
            ...
            backend.writeData(&profilesCtx, "signal_1d", "time", sig1d_val.data(), alconst::double_data, sig1d_dim, sig1d_size);
            ...
            backend.writeData(&timeSliceCtx, "ts_sig_0d", "time", &ts_0d_val, alconst::double_data, 0, nullptr);
            // ts_sig_1d
            ...
            backend.writeData(&timeSliceCtx, "ts_sig_1d", "time", ts_1d_val.data(), alconst::double_data, ts_1d_dim, ts_1d_size_arr);
        ...
        // Validate initial slice (t=0.0)
        {
             ...
             backend.readData(&profilesCtx, "signal_0d", "time", &data, &type, &dim, size);
             assert(dim == 0);
             ...
             assert(ts_size == 1);
             // ts_sig_0d
             backend.readData(&timeSliceCtx, "ts_sig_0d", "time", &data, &type, &dim, size);
             assert(dim == 0);
             ...
             free(data);
             // ts_sig_1d
             backend.readData(&timeSliceCtx, "ts_sig_1d", "time", &data, &type, &dim, size);
             assert(dim == 1);
