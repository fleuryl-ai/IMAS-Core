...
const std::string URI = "imas:hdf5?path=./test_db_timerange_nested_dynamic_aos_resampling";

int main() {
    try {
        std::cout << BOLD << "\n=== Test TimeRange Read with Nested Dynamic AoS (Resampling) ===\n" << RESET;

        const int time_dim_size = 20;
        const int aos_size = 2;

        // 1. Writing (same as the non-resampling test)
        {
            ...
            for (int i = 0; i < aos_size; ++i) {
                ...
                for (int t = 0; t < dyn_aos_size; ++t) {
                    ...
                    backend.writeData(&dynAosCtx, "signal_in_dyn_aos", "", &sig_val, alconst::double_data, 0, nullptr);
        ...
        // 2. Reading (Time Range with resampling)
        {
            std::cout << "\n--- Phase 2: Reading Time Range [0.55, 0.95] with resampling dt=0.2 ---\n";
            DataEntryContext dataEntryCtx(URI);
            HDF5Backend backend;
            ...
            double tmin = 0.55;
            double tmax = 0.95;
            std::vector<double> dtime = {0.2}; // Resampling with a step of 0.2
            ...
            for (int i = 0; i < read_aos_size; ++i) {
                ...
                std::cout << "    dynamic_aos size: " << read_dyn_aos_size << " (expected 3)\n";
                assert(read_dyn_aos_size == 3);
                ...
                for (int t = 0; t < read_dyn_aos_size; ++t) {
                    ...
                    backend.readData(&dynAosCtx, "signal_in_dyn_aos", "", &data_ptr, &datatype, &dim, size);
                    
                    assert(dim == 0);
                    double val = *(double*)data_ptr;
                    
                    double requested_time = tmin + t * dtime[0];
            ...
            std::cout << GREEN << "[OK] Time range read with resampling validation passed.\n" << RESET;
        }
