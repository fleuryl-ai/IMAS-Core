    ...
    try {
        ...
        // 1. Writing
        {
            ...
            for (int t = 0; t < time_steps; ++t) {
                ...
                backend.writeData(&profilesCtx, "signal_2d", "time", val_2d.data(), alconst::double_data, sig_dim, sig_size);
        ...
        // 2. Reading / Validation
        {
            ...
            for (int t = 0; t < time_steps; ++t) {
                // Validate signal_2d
                backend.readData(&profilesCtx, "signal_2d", "time", &data, &type, &dim, size);
                assert(dim == 2);
                assert(size[0] == dim1);
                assert(size[1] == dim2);
