...
void parse_varname(const std::string& varname, std::string& ids_name, int& occurrence, std::string& dataset_path) {
    size_t first_slash = varname.find('/');
    std::string prefix;
    if (first_slash != std::string::npos) {
        prefix = varname.substr(0, first_slash);
        dataset_path = varname.substr(first_slash + 1);
    } else {
        prefix = varname;
        dataset_path = "";
    }
...
// Structure for H5Literate callback
struct LinkSearchData {
    std::string target_suffix;
    std::string found_file_name;
};

herr_t find_elink_callback(hid_t loc_id, const char* name, const H5L_info_t* linfo, void* opdata) {
...
void resolve_source(const std::string& source, const std::string& ids_name, int occurrence, std::string& final_h5_file) {
